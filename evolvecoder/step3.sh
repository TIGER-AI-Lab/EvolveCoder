...
MAX_CONCURRENT=64
BATCH_DELAY=0.01
BASE_DIR="evolvecoder/outputs/experiment"

# -------------------------
# 第1轮初始输入文件（来自 step2.2）
# -------------------------
INPUT_FILE="${BASE_DIR}/gpt_4.1_mini/step2.2_merged_eval.jsonl"
...
for ROUND in $(seq 1 $NUM_ROUNDS); do
    ...
    python evolvecoder/step3.1_filter_tests.py \
        "${INPUT_FILE}" --round "${ROUND}" --num_proc "${NUM_PROC}"

    echo "👉 Step 3.2: Generate tests"
    python evolvecoder/step3.2_gen_tests.py \
        "${BASE_DIR}/gpt_4.1_mini/round_${ROUND}/step3.1_filter_tests_round_${ROUND}.jsonl" \
        --round "${ROUND}" \
    ...
    python evolvecoder/step3.3_parsing_tests.py \
    ...
    python evolvecoder/step3.5_gen_tests.py \
        "${BASE_DIR}/gpt_4.1_mini/round_${ROUND}/step3.4_filter_tests_round_${ROUND}.jsonl" \
        --round "${ROUND}" \
        --model_name "${MODEL_NAME}" \
        --save_batch_size "${SAVE_BATCH_SIZE}" \
        --max_concurrent 1 \
    ...
    python evolvecoder/step3.6_parsing_tests.py \
