...
def preprocess_prompts_auto(
    ...
    for item in data:
        ...
        if len(idx) < 5:
            idx = [i for i, m in sorted(enumerate(means), key=lambda x: x[1]) if m >= 0.1][:5]
        else:
            idx = random.sample(idx, 5)
        selected_tests = [tests[i] for i in idx]
        problem = item["problem"] + f"\nHere are 5 challenging test cases designed to evaluate your solution:\n{selected_tests}"
...
def main(
    ...
    **vllm_kwargs
    ...
    for item in data:
        qid = hash_messages(item['problem'])
...
"""
python acecoderv3/step3.6_vllm_gen.py acecoderv3/outputs/all_20/gpt_4.1_mini/step3.5_filter_solutions_round_1.jsonl \
    --start_idx=0 \
    --end_idx=50 \
    --save_batch_size=5 \
