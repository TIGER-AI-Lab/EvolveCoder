...
def print_statistics(data: List[dict], output_file: str = None):
    ...
    # Try to reshape only if solutions per problem is consistent
    if total_solutions % total_problems == 0 and solutions_per_problem > 0:
        pass_rates_per_problem_array = pass_rates_array.reshape(total_problems, -1)
        print(f"   Solutions per problem (uniform): {solutions_per_problem}")
        ...
        print(f"   Solutions per problem (variable): {total_solutions}/{total_problems} = {solutions_per_problem:.1f} avg")
        print(f"   Pass@k statistics skipped (non-uniform solution distribution)")
